...
<div class="event_decp_div hover_section agenda_section_admin">
    ...
    <button type="button" id="admin_agenda_add-agenda" class="admin_add_btn_all admin_agenda_add-agenda-button hover_section_content_show">Add Agenda</button>
        ...
        <div id="line-up-items-container">
            ...
            foreach ($line_up as $index => $item) {
                ...
                <div class="line-up-item" data-index="<?php echo $index; ?>">
                    ...
                    <input placeholder="Name" type="text" id="line_up_<?php echo $index; ?>_name" name="event_line_up[<?php echo $index; ?>][name]" value="<?php echo esc_attr($item['name']); ?>" />
...
<div class="event_decp_div hover_section extra_options_section">
    ...
    <p>Clarify your event's age limits and refund policy here to ensure attendees understand the guidelines, ensuring a seamless event experience.</p>
...
.admin_agenda_accordion-content {
    display: none; /* Content is initially hidden */
    padding: 10px;
    border-top: none;
    background-color: black!important;
    border-radius: 4px;
...
.event-section__form_link a {
    background-color: #555; /* Lighter grey for default step */
    color: white;
    padding: 10px 15px;
    border-radius:7px; /* Slightly rounded corners for the steps */
    text-decoration: none;
    position: relative; /* For the connecting lines */
    z-index: 1; /* To keep the links above the connecting lines */
...
.event-section__form_link a:not(:first-child):before {
    ...
    position: absolute;
    top: 50%;
    left: -42px; /* Adjust as needed based on your spacing */
    width: 42px; /* This should be double the left offset to reach the previous button */
