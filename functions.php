...
function display_upcoming_events_for_user_with_view_order_button() {
    ...
    // SVG loading animation HTML goes here
    echo '</div>';
    ...
    foreach ($customer_orders as $customer_order) {
        ...
        foreach ($items as $item_id => $item) {
            ...
            if (!in_array($event_id, $displayed_event_ids) && !empty($event_id)) {
                ...
                echo '<div class="ticketImage"><img src="' . esc_url($event_image_url) . '" alt="Event Image"></div>';
                echo '<div class="ticket_inner_div">';
                echo '<div class="ticketTitle">' . esc_html(truncate_title($event_title, 30)) . '</div>';
                if (!empty($event_address)) {
                    echo '<div class="eventaddress">' . esc_html($event_address) . ' <a class="opne_on_map_link" href="' . esc_url($map_link) . '" target="_blank">Open on Map</a></div>';
                }
                echo '<hr>';
                echo '<div class="ticketDetail">';
                echo '<div><span class="ticket-detail-title">Event Date:</span>&ensp;' . esc_html(date_i18n('F j, Y, g:i a', strtotime($event_start_date))) . '</div>';
                echo '<div><span class="ticket-detail-title">Ticket Quantity:</span>&ensp;' . esc_html($ticket_quantity) . '</div>';
                echo '<div><span class="ticket-detail-title">Order Total:</span><span class="woocommerce-Price-amount amount"><bdi>' . esc_html($order_total) . '</bdi></span></div>';
                echo '</div>';
                echo '</div>';
                ...
                echo '<div><span class="ticket-detail-title">Paid:</span>&ensp;' . esc_html($order_paid_date) . '</div>';
                echo '</div>';
