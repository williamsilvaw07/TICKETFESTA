...
function updateIndividualProgressCircles(ticketList) {
    $('.ticket-progress-container').each(function(index) {
        ...
        container.find('.progress-ring__circle').css({
...
function updateIndividualTicketInfo(ticketList) {
    ...
    $('.ticket-info_hidden_all li').each(function(index) {
        ...
        progressHtml += '<svg class="progress-ring" width="72" height="72">';
        progressHtml += '<circle class="progress-ring__circle-bg" cx="36" cy="36" r="31" stroke-width="6"></circle>';
        progressHtml += '<circle class="progress-ring__circle progress-ring__circle-individual" cx="36" cy="36" r="31" stroke-width="6"></circle>';
        progressHtml += '</svg>';
        progressHtml += '<div class="progress-percentage_individual"></div>'; // Percentage text
        progressHtml += '<div class="ticket-details">';
        progressHtml += '<div class="ticket-name">' + ticket.name + '</div>';
        progressHtml += '<div class="ticket-count">' + issued + ' issued out of ' + capacity + ' available</div>';
        progressHtml += '</div>';
        progressHtml += '</div>';
        ...
        // Update individual progress circle
        updateIndividualProgressCircle(container.find('.progress-ring__circle'), issued, capacity);
    });
}
...
function updateIndividualProgressCircle(container, issuedTickets, totalTickets) {
    ...
    container.closest('.ticket-progress-container').find('.progress-percentage_individual').text(precisePercentage + '%');
}
    ...
    ticketList.forEach(function(ticket) {
        ...
        var individualProgressHtml = `
            <div class="ticket-progress-container">
                ...
                <div class="progress-percentage_individual"></div>
