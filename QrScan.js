...
function updateIndividualProgressCircles(ticketList) {
    $('.ticket-progress-container').each(function(index) {
        ...
        container.find('.individual-progress-ring__circle').css({
            'stroke-dasharray': '194.779px',
            'stroke-dashoffset': '192.851px',
            'stroke': 'rgb(211, 250, 22)' // Color of progress
        });
...
function updateIndividualTicketInfo(ticketList) {
    ...
    $('.ticket-info_hidden_all li').each(function(index) {
        ...
        progressHtml += '<circle class="individual-progress-ring__circle" cx="36" cy="36" r="31" stroke-width="6"></circle>';
        progressHtml += '</svg>';
