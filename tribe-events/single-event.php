...
<script>
    ...
    ///FUNCTION TO REMOVE THE BR FROM TRH ADDRESS SECTION 

document.addEventListener('DOMContentLoaded', function() {
    // Select the <br> element within the context of the 'tribe-address' class
    var brElement = document.querySelector('.tribe-address br');

    // Check if the <br> element exists and remove it
    if (brElement) {
        brElement.remove();
    }
});
...
function recalculateTotal() {
    ...
    $('.tribe-tickets__tickets-item').each(function() {
        var item = $(this);
        var newPriceElement = item.find('.new-price');
        var priceElement = item.find('.tribe-amount');
        
        // Determine which price to use based on availability of new price
        var priceText = newPriceElement.length ? newPriceElement.text() : priceElement.text();
        var ticketAmount = parseFloat(priceText.replace(/[£,]/g, ''));
        // console.log("Parsed ticket amount:", ticketAmount);

        var quantity = parseInt(item.find('.tribe-tickets__tickets-item-quantity-number-input').val());
        // console.log("Quantity found:", quantity);

        if (!isNaN(ticketAmount) && !isNaN(quantity)) {
            total += ticketAmount * quantity;
            // console.log("Added to total:", ticketAmount * quantity);
        } else {
            // console.log("Invalid number found, skipping...");
        }
    });
...
/////FUNCTION TO APPLY THE COUPON TO THE TICKET PRICE 

    // Validate that coupon data is properly loaded
    if (!window.couponData || !Array.isArray(window.couponData.products)) {
        console.error('Coupon data or products array is undefined.');
        return; // Exit if data is not properly loaded
    }

    // Iterate over each ticket item on the page
    $('.tribe-tickets__tickets-item').each(function() {
        const item = $(this);
        const productName = item.find('.tribe-tickets__tickets-item-content-title').text().trim();

        // Check if the current product is eligible for the discount
        const applicableProduct = window.couponData.products.find(product => product.name === productName);
        if (applicableProduct) {
            updateTicketPrice(item, window.couponData);
        }
    });

    // Function to update the price of a ticket item
    function updateTicketPrice(item, couponData) {
        const priceElement = item.find('.tribe-tickets__tickets-item-extra .tribe-amount');
        const originalPriceText = priceElement.text();
        const originalPrice = parseFloat(originalPriceText.replace(/[£,]/g, ''));

        if (isNaN(originalPrice)) {
            console.error('Price format error:', originalPriceText);
            return; // Skip if the price format is incorrect
        }

        const newPrice = calculateDiscountedPrice(originalPrice, couponData);
        priceElement.html(`<span class="old-price">${couponData.currency_symbol}${originalPrice.toFixed(2)}</span> <span class="new-price">${couponData.currency_symbol}${newPrice.toFixed(2)}</span>`);
    }

    // Function to calculate discounted price based on the coupon type
    function calculateDiscountedPrice(price, couponData) {
        if (couponData.discount_type === 'fixed_cart') {
            return Math.max(0, price - couponData.amount);
        } else if (couponData.discount_type === 'percent') {
            return price * (1 - couponData.amount / 100);
        }
        return price; // Return original if no applicable discount type
    }
...
// Function to hide the currency symbol if there is a new price
function hideCurrencySymbol() {
    $('.tribe-amount').each(function() {
        var amountElement = $(this);
        var newPriceElement = amountElement.find('.new-price');

        if (newPriceElement.length > 0) {
            amountElement.closest('.tribe-formatted-currency-wrap').find('.tribe-currency-symbol').hide();
        } else {
            amountElement.closest('.tribe-formatted-currency-wrap').find('.tribe-currency-symbol').show();
        }
    });
}
...
// Function to add discount icon and class if there is a new price outside the sale price span
function addDiscountIcon() {
    $('.tribe-common-b2.tribe-common-b1--min-medium.tribe-tickets__tickets-item-extra-price').each(function() {
        var priceElement = $(this);
        var newPriceElement = priceElement.find('.new-price');

        if (newPriceElement.length > 0) {
            if (!priceElement.closest('.tribe-tickets__tickets-sale-price').length) {
                // Add a new div with discount icon, span, and icon
                priceElement.prepend('<div class="discount-icon"><span class="discount-label">Discount applied</span><i class="discount-indicator fas fa-tag"></i></div>');

                // Add class to the elements
                priceElement.addClass('has-discount');
                priceElement.find('.tribe-amount').addClass('has-discount');
            }
        }
    });
}
...
// Disable manual entry in the ticket quantity input fields
$('body').on('keydown touchstart', '.tribe-tickets__tickets-item-quantity-number-input', function(e) {
        // Prevent typing and touch events
        if (e.type === 'keydown' && !e.key.match(/ArrowLeft|ArrowRight|ArrowUp|ArrowDown|Backspace|Delete|Tab/)) {
            e.preventDefault();
        } else if (e.type === 'touchstart') {
            e.preventDefault();  // This will prevent touch events like focusing and scrolling on touch start
        }
    });

    // Disable mousewheel and touchmove changes on these inputs
    $('.tribe-tickets__tickets-item-quantity-number-input').on('wheel touchmove', function(e) {
        e.preventDefault();
    });

    // Blur input on focus to prevent the mobile keyboard from appearing
    $('.tribe-tickets__tickets-item-quantity-number-input').on('focus', function() {
        $(this).blur();
    });

    // Attach event listeners for increment and decrement buttons
    $('body').on('click', '.tribe-tickets__tickets-item-quantity-add, .tribe-tickets__tickets-item-quantity-remove', function() {
        // Use a timeout to handle after input updates
        setTimeout(function() {
            recalculateTotal(); // Assuming you have a function to recalculate totals
        }, 100);
    });
...
.tribe-tickets__tickets-item-extra-price .discount-icon{
    display: flex;
    flex-direction: row-reverse;
    align-content: center;
    align-items: center;
    gap: 10px;
    background-color: #3b2404;
    color: #eeb949;
...
.tribe-tickets__tickets-item-extra-price  .fa-tag:before {
    position: relative;
    top: 0px;
}
.tribe-tickets__tickets-item-extra .old-price{
    text-decoration: line-through;
    color: grey !important;
    font-weight: 300;
    font-size: 17px;
}
.tribe-tickets__tickets-item-extra .new-price{
